---
title: "Interations and Listcolumns"
author: "Tim Hauser"
...
Suppose we wanted to apply a different function, say median, to each column of list_norms. The chunk below includes both the loop and the map approach:
...
The map function returns a list; we could store the results as a new list column … !!! We’ve been using mutate to define a new variable in a data frame, especially one that is a function of an existing variable. That’s exactly what we will keep doing.
...
dynamite_reviews2 = map_df(vec_urls, read_page_reviews)
